...
I’m a Software Engineer with experience in C++ and Java development, focused on building reliable, maintainable, and performance-oriented software.
...
🐳 **Inception (Docker / System Infrastructure)**  
...
- **Minishell** → Unix shell built in C (processes, pipes, signals)
- **Cub3D** → Raycasting engine in C++ (graphics, rendering, optimization)
- **WebServer** → HTTP server in C++ (networking, concurrency, HTTP protocol)
- **Inception** → Docker-based infrastructure setup (services, networking, system orchestration)
